...
export default function ProfilePage() {
  ...
  const fullName = user ? `${user.firstName} ${user.lastName}` : 'Ahmed Hamza';
  const university = user?.university || 'Université Sesame';
  const email = user?.email || 'ahmed.hamza@sesame.tn';
  const phone = user?.phone || '+216 55 123 456';
  const userInitials = user?.firstName ? (user.firstName[0] + user.lastName[0]).toUpperCase() : 'AH';
    ...
    <div className="max-w-2xl mx-auto flex flex-col items-center gap-8 py-12">
      ...
      <section className="flex flex-col items-center">
        <div className="relative mb-6">
          ...
          <div className="absolute bottom-0 right-0 w-8 h-8 bg-emerald-500 rounded-full border-2 border-white flex items-center justify-center text-white shadow-lg">
        ...
        <div className="flex gap-2">
          <div className="flex items-center gap-1.5 bg-[#ecfdf5] text-[#065f46] px-3 py-1.5 rounded-full border border-[#d1fae5]">
            <GraduationCap size={14} />
            <span className="text-[11px] font-bold">Étudiant Vérifié</span>
          </div>
          <div className="flex items-center gap-1.5 bg-[#fefce8] text-[#854d0e] px-3 py-1.5 rounded-full border border-[#fef9c3]">
            ...
            <span className="text-[11px] font-bold">4.9/5 Confiance communauté</span>
          </div>
        </div>
      </section>
      ...
      <section className="w-full bg-white rounded-3xl p-6 border border-surface-container-highest shadow-sm space-y-4">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-on-surface-variant flex items-center gap-1">
                <BookOpen size={12} className="text-secondary" /> Établissement
              </label>
              <div className="bg-surface-container-low border border-surface-container rounded-xl p-4 font-medium text-primary shadow-sm uppercase">
                {university}
              </div>
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-on-surface-variant">Langue</label>
              <div className="bg-surface-container-low border border-surface-container rounded-xl p-4 font-medium text-primary shadow-sm uppercase">
                {user?.language || 'fr'}
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {['Colocation', 'Études', 'Sport', 'Technologie'].map(interest => (
      ...
      <section className="w-full bg-white rounded-3xl p-6 border border-surface-container-highest shadow-sm space-y-6">
        ...
        <div className="space-y-6 relative ml-4">
          ...
          <div className="flex gap-4 relative">
            ...
            <div>
              <h4 className="text-sm font-bold text-primary">Recherche de logement effectuée</h4>
              <p className="text-xs text-on-surface-variant font-medium">Filtres appliqués : Sesame, budget &lt; 700 DT</p>
            </div>
          </div>
          ...
          <div className="flex gap-4 relative">
            ...
            <div>
              <h4 className="text-sm font-bold text-primary">Demande de visite envoyée</h4>
              <p className="text-xs text-on-surface-variant font-medium">En attente de réponse du propriétaire</p>
            </div>
          </div>
        </div>
      </section>
