...
import { Search, Star, ShieldCheck, ArrowLeft, Eye, MessageSquare, CheckCircle2, Send } from 'lucide-react';
      ...
      <div className="max-w-2xl mx-auto flex flex-col gap-8 pb-10">
        ...
        <div className="p-1 bg-surface-container-low rounded-2xl flex border border-surface-container">
          <button 
            ...
            className={`flex-1 py-3 text-[11px] font-bold uppercase tracking-widest rounded-xl transition-all ${
              selectedRole === 'student' ? "bg-white shadow-sm text-secondary" : "text-on-surface-variant hover:bg-white/50"
            }`}
          >
            Colocataire
          </button>
          <button 
            ...
            className={`flex-1 py-3 text-[11px] font-bold uppercase tracking-widest rounded-xl transition-all ${
              selectedRole === 'owner' ? "bg-white shadow-sm text-secondary" : "text-on-surface-variant hover:bg-white/50"
            }`}
          >
            Propriétaire
          </button>
        ...
        <section className="space-y-4">
          ...
          <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
            {usersList.map((usr, idx) => {
              ...
              return (
                <motion.div 
                  ...
                  className={`w-full flex items-center gap-4 p-4 rounded-2xl text-left relative cursor-pointer border-2 transition-all ${
                    isSelected ? "bg-[#eff6ff] border-[#3b82f6]" : "bg-white border-surface-container hover:border-secondary/30"
                  }`}
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-[13px] text-on-surface-variant font-medium">
                      ...
                      {usr.university ? ` • ${usr.university}` : ''}
                      {isSelected && <span className="text-[#3b82f6] font-bold ml-1">• Voir plus</span>}
                    </p>
                  </div>
                  {isSelected && (
            ...
            {usersList.length === 0 && (
              <p className="text-center text-on-surface-variant italic py-6">Aucun profil correspondant trouvé.</p>
            )}
          </div>
        </section>
        ...
        {selectedUser && (
          <section className="bg-[#f2f4f6]/50 rounded-3xl p-6 border border-surface-container-highest space-y-6">
            <div className="flex items-center gap-4 cursor-pointer" onClick={() => navigate(`/student/${selectedUser._id}`)}>
              <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-white shadow-sm">
                <img 
                  src={selectedUser.role === 'student' 
        ...
        {selectedUser && (
          <section className="bg-white rounded-[32px] p-8 border border-surface-container-highest shadow-sm flex flex-col items-center gap-6">
            <h3 className="text-xl font-bold text-primary mt-2">Noter {userName}</h3>
            ...
            <button 
              ...
              className={`w-full bg-[#0051d5] text-white font-bold uppercase tracking-widest py-5 rounded-2xl shadow-xl shadow-secondary/20 hover:bg-[#0041ab] transition-all active:scale-[0.98] flex items-center justify-center gap-2 ${
